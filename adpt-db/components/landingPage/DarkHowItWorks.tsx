  ...
  return (
    <section className="relative py-16 bg-slate-950 overflow-hidden">
      ...
      <div className="absolute inset-0">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,var(--tw-gradient-stops))] from-cyan-900/10 via-transparent to-transparent" />
      </div>
          ...
          <div className="absolute top-16 left-0 right-0 h-px">
            {steps.map((_, index) => (
              index < steps.length - 1 && (
                <motion.div
                  key={index}
                  className="absolute h-px bg-linear-to-r from-cyan-500/50 to-blue-600/50"
                  style={{
          ...
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div className={`relative w-32 h-32 bg-linear-to-br ${step.gradient} rounded-2xl flex items-center justify-center mb-6`}>
                    <Icon className="w-12 h-12 text-white" />
                    
                    {/* Animated pulse */}
                    <motion.div
                      className={`absolute inset-0 bg-linear-to-br ${step.gradient} rounded-2xl opacity-0 group-hover:opacity-30`}
                  ...
                  {/* Glow */}
                  <div className={`absolute inset-0 bg-linear-to-br ${step.gradient} blur-2xl opacity-30 group-hover:opacity-50 transition-opacity duration-300 -z-10`} />
                </motion.div>
          ...
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                {/* Connecting Line */}
                {index < steps.length - 1 && (
                  <div className="absolute left-10 top-20 bottom-0 w-px bg-linear-to-b from-cyan-500/50 to-blue-600/50" />
                )}

                {/* Icon */}
                <div className="relative shrink-0">
                  <div className={`w-20 h-20 bg-linear-to-br ${step.gradient} rounded-xl flex items-center justify-center`}>
                    <Icon className="w-8 h-8 text-white" />
                  </div>
                  <div className={`absolute inset-0 bg-linear-to-br ${step.gradient} blur-xl opacity-40 -z-10`} />
